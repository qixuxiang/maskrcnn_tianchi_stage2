...
infer_path = '/home/hiudawn/python/seg/final/jinnan/jinnan2_round2_test_a_20190401'
images = glob.glob('{}/*.jpg'.format(infer_path))

normal_img_pth = './res.csv'
...
for image in images:
    ...
    predictions, confidence, boxes, labels, masks = coco_demo.run_on_opencv_image(img)

    
    if basename in normal_images[:-1]:
        ...
        for i in range(0, len(labels)):
            if(labels[i] == 1):
                #求numpy元素的并集
                masks_1 = np.array((masks_1+masks[i][0]) >= 1).astype(np.uint8)
            elif(labels[i] == 2):
                masks_2 = np.array((masks_2+masks[i][0]) >= 1).astype(np.uint8)    
            elif(labels[i] == 3):
                masks_3 = np.array((masks_3+masks[i][0]) >= 1).astype(np.uint8) 
            elif(labels[i] == 4):
                masks_4 = np.array((masks_4+masks[i][0]) >= 1).astype(np.uint8) 
            elif(labels[i] == 5):
                masks_5 = np.array((masks_5+masks[i][0]) >= 1).astype(np.uint8) 
